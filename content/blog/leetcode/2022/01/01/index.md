...
前半部的爆破所獲錢幣最大值是 $M(i,k-1)$，後半部的最大值是 $M(k+1,j)$（在邊緣的案例上，譬如，$k = i$ 的時候 $M(i, i-1) = 0$，因為 $D$ 是一個上半矩陣，同理，$M(k(=j) + 1, j ) = 0$），最後加上爆破 $k$ 所獲得的錢幣，因為這兩段都爆破了，還未爆破的左邊現在是 $n_{i-1}$，右邊則是 $n_{j+1}$。
