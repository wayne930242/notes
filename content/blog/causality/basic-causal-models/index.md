...
如果 $\mathbf{V}$ 是非離散的，透過對結構等式的函數進行形式上的假設，我們可能得出比馬可夫等價類更好的結論。 
...
令內生變數 $\mathbf{V}=\{X_1, X_2,...,X_n\}$ 與外生變數 $\mathbf{V} =\{ U_1,U_2,...,U_n \}$，結構等式的形式是 $x_i = f_i(pa_i,u_i)$。$P'$ 是在 $\mathbf{U}$ 上的機率分配，可以推出 $\mathbf{U}\cup \mathbf{V}$ 上的機率分配 $P$。
