...
* 分解條件等價於**馬可夫屏蔽條件**（Pearl 1988）：對於所有 $X_i\in V$ 以及 $Y\subset \{ Z| Z\in V \text{且 } Z \text{ 不在 } X \text{的後代節點} \}$，則 $P(x_i|pa_i, y)=P(x_i|pa_i)$。
...
**因果貝式網絡**：$P$ 是在 $\mathbf{V}$ 上的機率分配，干預分配 $P_x(v)$ 表示 $do(X=x)$ 干預而來的機率分配。令 $P_*\coloneqq \{ P_x(v) | X \subset \mathbf{V}\text{ , x 是 X 範圍內的常數} \}$ 。DAG $G$ 是相容 $P_*$ 的因果貝式網絡的條件是，對於所有 $P_x \in P_*$：
...
* 將因果網絡表示成一系列的等式，形式為：$x_i=f_i(pa_i, u_i)$，$i=1,...,n$。$V = \{X_i|i=1,...,n\}$ 稱之為內生變數（endogenous variables）的集合。
* 其中 $U_i$ 代表隱性變數，或誤差，或外生變數（exogenous variables）的集合。
* 外生變數不在 DAG 圖形上，與相容於 $P$ 的 DAG 中的節點以 $\leftrightarrow$ 連接，形成 ADMG。
* 透過結構等式，我們可以更容易計算干預造成的結果，而不需要計算出所有的干預分配。
* 不難看出，變數的機率由誤差提供，並且會因為干預改變而改變分配。
* 馬可夫條件定理（Pearl and Verma 1991）：假設如果誤差變數 $U_i$ 在 $P$ 中是獨立的，則在 $V$ 上的機率分配滿足馬可夫分解條件。
...
如果 $V$ 是非離散的，透過對結構等式的函數進行形式上的假設，我們可能得出比馬可夫等價類更好的結論。 
...
**後門判準**：若 $X$ 和 $Y$ 是在 $V$ 中的變數，且 $Z\subset V-\{X,Y\}$，使得
